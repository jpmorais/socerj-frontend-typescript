...
const PatrocinadoresDelete = () => {
  ...
  const { mutate, isPending: isPendingPatch } =
    Patrocinadores.deletePatrocinador({
      onSuccess: () => {
      ...
      onError: (error: any) => {
        toast.error(
          `falha ao excluir patrocinador: ${error?.response?.data?.message}`
        );
        navigate("..");
      },
    });
    ...
    <Modal>
      <form onSubmit={handleSubmit}>
        ...
        <h3 className="font-bold text-lg my-8">
          Excluir #{data?.id} | {data?.patrocinador}
        </h3>
...
export default PatrocinadoresDelete;
